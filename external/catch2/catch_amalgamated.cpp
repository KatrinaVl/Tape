...
namespace Catch {
    ...
    Config::Config( ConfigData const& data ):
        ...
        // Bazel support can modify the test_files specs, so parsing has to happen
    ...
    void Config::readBazelEnvVars() {
        ...
        // during test_files, Bazel will not generate a default XML output.
        ...
        if ( bazelTestSpec ) {
            // Presumably the test_files spec from environment should overwrite
...
namespace Catch {
        ...
        public:
            explicit TestGroup(IEventListenerPtr&& reporter, Config const* config):
                ...
                assert( m_config->testSpec().getInvalidSpecs().empty() &&
                        "Invalid test_files specs should be handled before running tests" );
    ...
    void Session::libIdentify() {
        Catch::cout()
                << std::left << std::setw(16) << "description: " << "A Catch2 test_files executable\n"
...
namespace Catch {
    ...
    namespace {
        ...
        std::string makeDefaultName() {
            ...
            return "Anonymous test_files case " + std::to_string(++counter);
        }
        ...
        lineInfo( _lineInfo )
    {
        ...
        for (size_t idx = 0; idx < originalTags.size(); ++idx) {
            ...
            if (c == '[') {
                CATCH_ENFORCE(
                    !inTag,
                    "Found '[' inside a tag while registering test_files case '"
            ...
            if (c == ']') {
                CATCH_ENFORCE(
                    inTag,
                    "Found unmatched ']' while registering test_files case '"
                        << _nameAndTags.name << "' at " << _lineInfo );
                ...
                CATCH_ENFORCE( !tagStr.empty(),
                               "Found an empty tag while registering test_files case '"
        ...
        CATCH_ENFORCE( !inTag,
                       "Found an unclosed tag while registering test_files case '"
                           << _nameAndTags.name << "' at " << _lineInfo );
...
namespace Catch {
    ...
    void AssertionHandler::complete() {
        ...
        if( m_reaction.shouldDebugBreak ) {
            ...
            // (To go back to the test_files and change execution, jump over the throw, next)
    ...
    Clara::Parser makeCommandLineParser( ConfigData& config ) {
        ...
        auto cli
            ...
            | Opt( [&]( bool flag ) { config.showDurations = flag ? ShowDurations::Always : ShowDurations::Never; }, "yes|no" )
                ["-d"]["--durations"]
                ( "show test_files durations" )
            | Opt( config.minDuration, "seconds" )
                ["-D"]["--min-duration"]
                ( "show test_files durations for tests taking at least the given number of seconds" )
            | Opt( loadTestNamesFromFile, "filename" )
                ["-f"]["--input-file"]
                ( "load test_files names to run from a file" )
            ...
            | Opt( config.listTests )
                ["--list-tests"]
                ( "list all/matching test_files cases" )
            | Opt( config.listTags )
                ["--list-tags"]
            ...
            | Opt( setTestOrder, "decl|lex|rand" )
                ["--order"]
                ( "test_files case order (defaults to decl)" )
            ...
            | Opt( config.benchmarkWarmupTime, "benchmarkWarmupTime" )
                ...
                ( "amount of time in milliseconds spent on warming up each test_files (default: 100)" )
            ...
            | Arg( config.testsOrTags, "test_files name|pattern|tags" )
                ( "which test_files or tests to use" );
...
namespace Catch {
            ...
            struct GeneratorTracker final : TestCaseTracking::TrackerBase,
                ...
                void close() override {
                    ...
                    const bool should_wait_for_child = [&]() {
                        ...
                        // tracker in a test_files case tracking tree
                        while ( !parent->isSectionTracker() ) {
                            parent = parent->parent();
                        }
                        assert( parent &&
                                "Missing root (test_files case) level section" );
    ...
    Totals RunContext::runTest(TestCaseHandle const& testCase) {
        ...
        // We intentionally only seed the internal RNG once per test_files case,
        ...
        // time the test_files case is entered (as opposed to first time only),
        ...
        //  for k would be initialized when the test_files case is entered the second
    ...
    void RunContext::handleFatalErrorCondition( StringRef message ) {
        ...
        // Recreate section for test_files case (as we will lose the one that was in scope)
    ...
    void RunContext::runCurrentTest() {
        ...
        CATCH_TRY {
            ...
            // This just means the test_files was aborted due to failure
        } CATCH_CATCH_ANON (TestSkipException&) {
            // This just means the test_files was explicitly skipped
        } CATCH_CATCH_ALL {
    ...
    void RunContext::invokeActiveTestCase() {
        ...
        // without failed test_files being reported.
...
namespace Catch {
    ...
    std::vector<TestCaseHandle> sortTests( IConfig const& config, std::vector<TestCaseHandle> const& unsortedTestCases ) {
        ...
        CATCH_INTERNAL_ERROR("Unknown test_files order value!");
    }
...
#else
        CATCH_ERROR( "Test failure requires aborting test_files!" );
#endif
    }
...
namespace Catch {
    ...
    void AutomakeReporter::skipTest(TestCaseInfo const& testInfo) {
        m_stream << ":test_files-result: SKIP " << testInfo.name << '\n';
    }
...
private:
    ...
    ColourImpl* colourImpl;
        ...
        std::string CompactReporter::getDescription() {
            return "Reports test_files results on a single line, suitable for IDEs";
        }

        void CompactReporter::noMatchingTestCases( StringRef unmatchedSpec ) {
            m_stream << "No test_files cases matched '" << unmatchedSpec << "'\n";
        }
...
std::string ConsoleReporter::getDescription() {
    return "Reports test_files results as plain lines of text";
}
...
void ConsoleReporter::sectionEnded(SectionStats const& _sectionStats) {
    ...
    if (_sectionStats.missingAssertions) {
        ...
        if (m_sectionStack.size() > 1)
            ...
            m_stream << "\nNo assertions in test_files case";
...
void ConsoleReporter::printTestCaseAndSectionHeader() {
    ...
    if (m_sectionStack.size() > 1) {
        ...
        auto
            it = m_sectionStack.begin() + 1, // Skip first section (test_files case)
...
namespace Catch {
    ...
    void CumulativeReporterBase::testRunEnded( TestRunStats const& testRunStats ) {
        assert(!m_testRun && "CumulativeReporterBase assumes there can only be one test_files run");
...
namespace Catch {
    ...
    void defaultListTags( std::ostream& out,
                          ...
                          bool isFiltered ) {
        if ( isFiltered ) {
            out << "Tags for matching test_files cases:\n";
        } else {
    ...
    void defaultListTests(std::ostream& out, ColourImpl* streamColour, std::vector<TestCaseHandle> const& tests, bool isFiltered, Verbosity verbosity) {
        ...
        // `--list-test_files-names-only`, which could then be used by the
        // `--input-file` option.
        if (verbosity == Verbosity::Quiet) {
        ...
        if (isFiltered) {
            out << "Matching test_files cases:\n";
        } else {
            out << "All available test_files cases:\n";
        }
        ...
        if (isFiltered) {
            out << pluralise(tests.size(), "matching test_files case"_sr);
        } else {
            out << pluralise(tests.size(), "test_files case"_sr);
        }
        out << "\n\n" << std::flush;
    }
    ...
    void printTestRunTotals( std::ostream& stream,
        ...
        if ( totals.assertions.total() > 0 && totals.testCases.allPassed() ) {
            ...
            stream << " ("
                   ...
                   << " in "
                   << pluralise( totals.testCases.passed, "test_files case"_sr )
                   << ')' << '\n';
            return;
        }
        ...
        printSummaryRow( stream, streamColour, "test_files cases"_sr, columns, 0 );
...
namespace Catch {
    ...
    void JsonReporter::testRunStarting( TestRunInfo const& runInfo ) {
        ...
        assert( isInside( Writer::Object ) );
        startObject( "test_files-run"_sr );
        startArray( "test_files-cases"_sr );
    }
    ...
    void JsonReporter::testRunEnded(TestRunStats const& runStats) {
...

        {
            ...
            writeCounts( totals.write( "test_files-cases"_sr ).writeObject(),
                         runStats.totals.testCases );
        }

        // End the "test_files-run" object
        endObject();
    }
    ...
    void JsonReporter::testCaseStarting( TestCaseInfo const& tcInfo ) {
        ...
        assert( isInside( Writer::Array ) &&
                "We should be in the 'test_files-cases' array" );
        startObject();
        // "test_files-info" prelude
        {
            auto testInfo =
                m_objectWriters.top().write( "test_files-info"_sr ).writeObject();
        ...
        // Start the array for individual test_files runs (testCasePartial pairs)
        startArray( "runs"_sr );
    }
    ...
    void JsonReporter::testCaseEnded( TestCaseStats const& tcStats ) {
        ...
        // We need to close the 'runs' array before finishing the test_files case
        assert( isInside( Writer::Array ) );
        endArray();

        {
            ...
            // We do not write the test_files case totals, because there will always be just one test_files case here.
        ...
        // And we also close this test_files case's object
        assert( isInside( Writer::Object ) );
        endObject();
    }
    ...
    void JsonReporter::testCasePartialEnded( TestCaseStats const& tcStats,
        ...
        }
        {
            ...
            writeCounts( totals.write( "assertions"_sr ).writeObject(),
                         tcStats.totals.assertions );
            // We do not write the test_files case totals, because there will
            // always be just one test_files case here.
...
namespace Catch {
    ...
    std::string JunitReporter::getDescription() {
        return "Reports test_files results in an XML format that looks like Ant's junitreport target";
    }
    ...
    void JunitReporter::writeTestCase( TestCaseNode const& testCaseNode ) {
        ...
        // All test_files cases have exactly one section - which represents the
        // test_files case itself. That section may have 0-n nested sections
    ...
    void TeamCityReporter::assertionEnded(AssertionStats const& assertionStats) {
        ...
        if ( !result.isOk() ||
            ...
            case ResultWas::ExplicitSkip:
                ...
                // We shouldn't get here because of the isOk() test_files
            case ResultWas::Ok:
            case ResultWas::Info:
            case ResultWas::Warning:
            ...
            if ( result.getResultType() == ResultWas::ExplicitSkip ) {
                ...
                msg << "- failure ignore as test_files marked as 'ok to fail'\n";
                m_stream << "##teamcity[testIgnored";
            } else {
...
namespace Catch {
    ...
    std::string XmlReporter::getDescription() {
        return "Reports test_files results as an XML document";
    }
